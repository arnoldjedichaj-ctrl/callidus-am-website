# -*- coding: utf-8 -*-
"""
Create a German male voiceover audio from the provided text.

Output:
- no background music
- no waves
- strong, clear narration style
"""
...
SEG_DIR = BASE / "klare_gedanken_male_segments"
OUT_VOICE = BASE / "kraft-klarer-gedanken-male-orus.wav"
OUT_MP3 = BASE / "kraft-klarer-gedanken-male-orus.mp3"
OUT_META = BASE / "kraft-klarer-gedanken-male-orus.meta.json"
...
Klare Gedanken helfen uns, zu erkennen, was wir wirklich wollen, worauf wir achten und welche Handlung als nächstes sinnvoll ist.""", 2.4),
...
Ein Wunsch allein reicht jedoch nicht. Wir müssen wissen, was wir wirklich möchten, und bereit sein, etwas dafür zu tun.""", 2.2),
...
Je klarer dieses Bild ist, desto leichter kann sich Ihre Aufmerksamkeit darauf richten.""", 2.6),
...
Angst und ständiger Zweifel können dagegen dazu führen, dass wir uns selbst blockieren.""", 2.6),
...
Der Satz soll Sie an Ihr Ziel erinnern und an das Gefühl, mit dem Sie dieses Ziel verbinden.""", 2.4),
...
Die Übung soll ruhig und ohne Druck erfolgen.""", 2.8),
...
Häuser schützen vor Kälte und Regen. Heizungen sorgen für Wärme. Klimaanlagen helfen bei großer Hitze.""", 2.4),
...
Ein Wunsch ist deshalb nicht grundsätzlich egoistisch. Entscheidend ist, wie wir mit ihm umgehen.""", 2.4),
...
Es ist weder gut noch schlecht. Entscheidend ist, wofür wir es verwenden.""", 2.4),
...
Dadurch kann ein Nutzen entstehen, von dem mehrere Menschen profitieren.""", 2.5),
...
Doch der Erfolg eines anderen bedeutet nicht automatisch, dass für Sie weniger übrig bleibt.""", 2.6),
...
Sie können aus der Vergangenheit lernen. Sie sollten aber nicht jeden neuen Schritt danach beurteilen, was früher schiefgegangen ist.""", 2.8),
...
Der festigt diese Sicht auf sich selbst.""", 2.5),
...
Sie beeinflussen aber, wie wir uns selbst sehen und welche Möglichkeiten wir wahrnehmen.""", 2.6),
...
Diese widersprüchlichen Gedanken können sich gegenseitig blockieren.""", 2.8),
...
Sie richtet den Blick zusätzlich auf das, was bereits vorhanden ist.""", 2.8),
...
Vertrauen kann uns Mut geben. Doch Ziele brauchen meistens auch Entscheidungen, Übung und konkretes Handeln.""", 2.8),
...
Bleiben Sie offen für Möglichkeiten und richten Sie Ihr Handeln auf Ihr Ziel aus.""", 3.2),
...
Und gehen Sie ihn mit ruhigem, klarem Denken.""", 2.0),
]
...
def generate_tts(text: str, output_path: Path, keys: list[str]) -> None:
    ...
    prompt = (
        "Sprich auf Deutsch mit einer kraftvollen, klaren, männlich wirkenden Erzählerstimme. "
        "Der Stil ist ruhig, bestimmt, hochwertig und sachlich inspirierend. "
        "Nicht meditativ flüstern, nicht übertreiben, keine Zusatzkommentare. "
        "Lies nur diesen Text: "
        + text
    )
    payload = {
...
def export_mp3() -> None:
    base.run_ffmpeg([
        ...
        "-b:a", "192k",
        "-id3v2_version", "3",
        "-metadata", "title=Die Kraft klarer Gedanken",
...
def write_metadata(duration: float) -> None:
    meta = {
        ...
        "style": "Male, strong, clear narration. Intro and ending added.",
        "files": {
