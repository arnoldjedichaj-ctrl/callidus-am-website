#!/usr/bin/env python3
"""
Callidus A&M – YouTube Automation mit Telegram-Freigabe
Täglich auf Synology DS218 via Task Scheduler

Modi:
  python main.py         – Einmalig ausführen (für Cron/Task Scheduler)
  python main.py --bot   – Dauerhafter Bot-Modus mit manuellem /jetzt Befehl
"""

import os, json, random, datetime, subprocess, sys, logging, time, tempfile, shutil, wave, base64, requests

# ─── KONFIGURATION ────────────────────────────────────────────────
# CALLIDUS_MAIN_VERSION_MARKER=GROWTH_CTA_KARAOKE_OFF_2026_06_11
...
TELEGRAM_TOKEN = "8532341668:AAFXXXUrpTQ8WCKGB6eBNXysj26BuOeHM68"
...
# ─── FIREBASE / CALLIDUS TV (Nexus App) ───────────────────────────
...
CALLIDUS_TV_MAX      = 30   # Maximale Videos in "Callidus TV" – aelteste fliegen raus

CROSSFADE_DURATION = 1.5   # Sekunden Crossfade zwischen Bildern
...
AI_IMAGES_FIRST    = True  # True = KI-Bilder (Pollinations) als Primaerquelle, Pexels nur Fallback
KEN_BURNS          = True  # True = langsame Zoom/Schwenk-Bewegung auf KI-Bildern (Video-Look)

# Tägliche automatische Ausführung im Bot-Modus
DAILY_HOUR   = 10
DAILY_MINUTE = 0
POSTING_TAGE = {6}  # Sonntag=6 (0=Montag) – woechentliches Langvideo So 10:00 Uhr

# Satire-taugliche Themen (Momus-Stil): jedes Thema beschreibt eine moderne
# (Un-)Gewohnheit oder einen Mythos, der erst ironisch gefeiert und danach
# wissenschaftlich aufgeklaert wird. Bewusst breit gemischt (Geist, Koerper,
# Ernaehrung, Schlaf, Bewegung, Digital) fuer abwechslungsreiche Geschichten.
THEMEN = [
...
# ─── KI-BILD KUNSTSTILE (kostenlos via Pollinations/FLUX) ─────────
# Wechselnder Kunststil pro Woche → abwechslungsreicher, nicht-fotorealistischer
# Look, der die Satire traegt. Wird ueber einen Datum-Seed rotiert.
ART_STYLES = [
    {
        "label":  "Anime",
        "prefix": ("modern Japanese anime style, clean cel shading, expressive characters, "
    ...
    {
        "label":  "Editorial Flat Illustration",
        "prefix": ("modern flat vector editorial illustration, bold geometric shapes, "
                   "limited harmonious color palette, clean minimal composition, subtle grain, "
                   "magazine illustration style, NOT photorealistic, no real photo, "),
    },
]

def todays_art_style():
    """Waehlt den Kunststil der Woche anhand der Kalenderwoche (rotierend)."""
...
logging.basicConfig(
    filename=f"{LOGS_DIR}/workflow.log",
    level=logging.INFO,
...
Schreibe ein YouTube-Video-Script (4-5 Minuten) zum Thema: "{thema}"

DER MOMUS-STIL = SATIRE + ANSCHLIESSENDE AUFKLAERUNG (zwei klar getrennte Teile):

TEIL 1 – DIE SATIRE (Slides 1-5): Beissende, ironische Lobrede auf das FALSCHE Verhalten.
- Erzaehler-Haltung: ein zynischer "Effizienz-Coach", der die schlechte Gewohnheit als geniales Premium-Upgrade verkauft.
- Anrede in TEIL 1: hoefliches, distanziertes "Sie" ("Haben Sie heute schon...?").
- Trockener Humor, Uebertreibung, scheinheilige Ratschlaege. Lobe genau das, was dem Menschen schadet.
- Sprachbilder wie im Momus-Magazin: moderne Metaphern (Algorithmus, Upgrade, Effizienz, Optimierung, Ich.exe).
- NIE platt oder beleidigend – elegant, intelligent, augenzwinkernd. Der Zuschauer soll schmunzeln UND sich ertappt fuehlen.

TEIL 2 – DIE AUFKLAERUNG (Slides 6-9): Harter Schnitt zur Wahrheit.
- Wechsel auf warmes, direktes "du/dein". Jetzt ehrlich, fundiert, motivierend.
- Echte Wissenschaft mit konkreten Zahlen/Mechanismen (Hormone, Gehirnareale, Prozentwerte, Studienlage) – plausibel und serioes, nichts erfinden was unrealistisch wirkt.
- Ende mit echter, positiver Motivation, einem umsetzbaren ersten Schritt und einem KLAREN, sichtbaren Callidus-CTA. Der letzte Slide muss einen Produktpfad ausdrücklich nennen: NEXUS App, Stress Reset Kurs oder Callidus Empfehlungen. Keine Heilversprechen, keine aggressive Werbung.

WICHTIG – ABWECHSLUNG (Seed {heute_iso}):
- Variiere Einstieg und Stimmung STARK von Ausgabe zu Ausgabe – NICHT immer derselbe (z.B. nicht immer "traurig" oder "muede"). Mal frech, mal absurd, mal als fingierte Werbung, mal als Mini-Szene mit einer Figur, mal als ironische Anleitung.
- Erfinde bei Bedarf eine konkrete, frische Figur/Situation als Aufhaenger.

FORMALE VORGABEN:
- GENAU 9 Slides (je ca. 30-35 Sekunden = 4-5 Min gesamt).
- Sprechtext pro Slide: 90-110 Woerter.
- motivationstext: kurzer Mitlese-Satz (max 70 Zeichen!) OHNE Emojis. In TEIL 1 ironisch ("Pixel fordern nichts ausser Akkulaufzeit."), in TEIL 2 ehrlich-motivierend ("Ein echtes Gespraech senkt dein Cortisol sofort.").
- bildsuche: englischer Bildprompt, der zur Szene passt (Figuren/Handlung, nicht nur Deko).
- Keine Beratungsangebote, keine Kontaktaufforderungen.

SLIDE-DRAMATURGIE:
- Slide 1 (SATIRE-HOOK): Provokante ironische Eroeffnung, die das schlechte Verhalten feiert. Neugier erzwingen.
- Slide 2 (SATIRE-AUFBAU): Warum dieses Verhalten angeblich so klug/modern/effizient ist.
- Slide 3 (SATIRE-STEIGERUNG): Ironie zuspitzen, absurde "Vorteile" auflisten.
- Slide 4 (SATIRE-HOEHEPUNKT): Die zynische Lobrede auf die Spitze treiben.
- Slide 5 (DER BRUCH): Eine ehrliche Uebergangsfrage, die das Lachen ins Nachdenken kippt ("Fuehlt sich diese 'Freiheit' manchmal seltsam hohl an? Das ist kein Zufall.").
- Slide 6 (AUFKLAERUNG START): Was im Koerper/Gehirn WIRKLICH passiert – erster Fakt mit Zahl.
- Slide 7 (AUFKLAERUNG VERTIEFUNG): Mechanismus + Folgen, weiterer konkreter Fakt.
- Slide 8 (DIE GUTE NACHRICHT): Loesung/Umkehrbarkeit, der staerkste positive Fakt.
- Slide 9 (MOTIVATION + CTA): Warmer Schlussappell + konkreter erster Schritt. Dann KLAR sichtbar EIN Produktpfad: NEXUS App fuer persoenlichen Gesundheitsplan, Stress Reset bei Stress-Themen, Empfehlungen bei Supplement-/Produkt-Themen. Titel muss produktnah sein, z.B. "NEXUS TESTEN", "STRESS RESET" oder "EMPFEHLUNGEN".

ZUSAETZLICH fuer das Thumbnail (kreatives YouTube-Vorschaubild):
- thumbnail_text: 2-5 Woerter, GROSS, plakativ, neugierig machend, OHNE Emoji (z.B. "ERSCHOEPFUNG = ERFOLG?" oder "DAS LUEGT DEIN GEHIRN").
- thumbnail_bildsuche: englischer Bildprompt fuer ein ausdrucksstarkes, emotionales Hauptmotiv des Thumbnails (eine Figur/Szene mit klarer Mimik).

JSON Format:
{{
  "titel": "Packender, leicht ironischer Titel mit 1 Emoji",
  "beschreibung": "YouTube Beschreibung 2-3 Saetze (Satire-Teaser + Aufklaerungs-Versprechen) mit Hashtags",
  "tags": ["tag1","tag2","tag3","tag4","tag5"],
  "thumbnail_text": "MAX 5 WOERTER",
  "thumbnail_bildsuche": "English image prompt for an expressive thumbnail subject",
  "slides": [
    {{"titel": "Satire-Hook",   "sprechtext": "SATIRE (Sie-Ton): ironische Eroeffnung, schlechtes Verhalten feiern - 90-110 Woerter",  "bildsuche": "English search term", "motivationstext": "Ironischer Satz max 70 Zeichen"}},
    {{"titel": "Aufbau",        "sprechtext": "SATIRE: warum es angeblich so klug ist - 90-110 Woerter",                                  "bildsuche": "English search term", "motivationstext": "..."}},
    {{"titel": "Steigerung",    "sprechtext": "SATIRE: absurde Vorteile zuspitzen - 90-110 Woerter",                                      "bildsuche": "English search term", "motivationstext": "..."}},
    {{"titel": "Hoehepunkt",    "sprechtext": "SATIRE: zynische Lobrede auf die Spitze treiben - 90-110 Woerter",                         "bildsuche": "English search term", "motivationstext": "..."}},
    {{"titel": "Der Bruch",     "sprechtext": "UEBERGANG: ehrliche Frage, Lachen kippt in Nachdenken - 90-110 Woerter",                  "bildsuche": "English search term", "motivationstext": "..."}},
    {{"titel": "Aufklaerung",   "sprechtext": "AUFKLAERUNG (du-Ton): was wirklich im Koerper passiert, erster Fakt mit Zahl - 90-110 Woerter", "bildsuche": "English search term", "motivationstext": "..."}},
    {{"titel": "Vertiefung",    "sprechtext": "AUFKLAERUNG: Mechanismus + Folgen, konkreter Fakt - 90-110 Woerter",                      "bildsuche": "English search term", "motivationstext": "..."}},
    {{"titel": "Gute Nachricht","sprechtext": "AUFKLAERUNG: Loesung/Umkehrbarkeit, staerkster positiver Fakt - 90-110 Woerter",          "bildsuche": "English search term", "motivationstext": "..."}},
    {{"titel": "NEXUS TESTEN","sprechtext": "MOTIVATION + CTA: warmer Schlussappell, erster Schritt. Dann klar sichtbar ein passender Produktpfad: NEXUS App, Stress Reset Kurs oder transparente Empfehlungen auf callidus-am.de. Formuliere konkret, nicht allgemein, ohne Heilversprechen.", "bildsuche": "person taking a positive first step, hopeful", "motivationstext": "NEXUS, Stress Reset oder Empfehlungen: dein nächster Schritt."}}
  ]
}}
...
def build_segment_with_crossfade(image_paths, audio_path, seg_out, fmt, crossfade=1.5,
    ...
    for idx, img_path in enumerate(image_paths):
        ...
        vf  = f"{scale_crop},fps=25,format=yuv420p"
        cmd = [FFMPEG, "-y",
...
def _kenburns_vf(fmt, duration, idx, target_w, target_h):
    """Liefert einen Ken-Burns-zoompan-Filter (langsamer Zoom/Schwenk) fuer ein
    Standbild, damit KI-Bilder wie Video wirken. Richtung variiert pro idx.
    Hochskalieren vor zoompan vermeidet Pixel-Jitter."""
    frames = max(1, int(round((duration + 0.5) * 25)))
    up_w, up_h = int(target_w * 1.5), int(target_h * 1.5)
    variant = idx % 4
    if variant == 0:      # Zoom-in, zentriert
        z = "z='min(zoom+0.0009,1.20)'"
        x = "x='iw/2-(iw/zoom/2)'"; y = "y='ih/2-(ih/zoom/2)'"
    elif variant == 1:    # Zoom-out (von 1.20 zurueck)
        z = "z='if(eq(on,1),1.20,max(1.001,zoom-0.0009))'"
        x = "x='iw/2-(iw/zoom/2)'"; y = "y='ih/2-(ih/zoom/2)'"
    elif variant == 2:    # Zoom-in + Schwenk nach rechts
        z = "z='min(zoom+0.0008,1.18)'"
        x = "x='(iw-iw/zoom)*on/{0}'".format(frames); y = "y='ih/2-(ih/zoom/2)'"
    else:                 # Zoom-in + Schwenk nach unten
        z = "z='min(zoom+0.0008,1.18)'"
        x = "x='iw/2-(iw/zoom/2)'"; y = "y='(ih-ih/zoom)*on/{0}'".format(frames)
    return (f"scale={up_w}:{up_h}:force_original_aspect_ratio=increase,"
            f"crop={up_w}:{up_h},"
            f"zoompan={z}:{x}:{y}:d={frames}:s={target_w}x{target_h}:fps=25,"
    ...
    for idx, (ptype, ppath) in enumerate(pieces):
        ...
        try:
            if ptype == "video":
                ...
                # Ken-Burns-Bewegung (zoompan) macht das KI-Standbild zur Video-Variante.
                if KEN_BURNS:
                    img_vf = _kenburns_vf(fmt, piece_dur, idx, W, H)
                else:
                    img_vf = f"{scale_crop},format=yuv420p"
...
def create_logo_outro(work_dir, fmt):
    ...
    draw.text((cx, y2), "NEXUS • Stress Reset • Empfehlungen", fill=MID_GREEN, font=font_small, anchor="mm")
...
def mix_background_music(video_path, final_out):
    ...
    if not music_files:
        log.info("Keine unblockierte Musikdatei gefunden – ohne Musik")
...
def build_video(script, fmt, work_dir):
    ...
    art           = todays_art_style()  # Wochen-Kunststil fuer KI-Bilder
    log.info(f"KI-Bild-Stil dieser Woche: {art['label']}")
    ...
    for i, slide in enumerate(slides):
        ...
        # Mitlese-/Motivationstext-Overlay deaktiviert: keine Ablese-Texte mehr im Video.
        mot_png_cf = ""
        ...
        if AI_IMAGES_FIRST:
            ...
            # Mehrere Bilder pro Slide im Wochen-Kunststil, danach Ken-Burns-Bewegung.
            ai_w, ai_h = target
            n_ai = min(num_imgs, IMAGES_PER_SLIDE + 1)
            for j in range(n_ai):
                ai_prompt = (f"{art['prefix']}{bildsuche}, "
                             f"clear focal subject, expressive, cinematic composition, "
            ...
            if saved_imgs:
                ...
                # KI fehlgeschlagen → Pexels als Fallback
                log.warning(f"Segment {i}: KI-Bilder fehlgeschlagen → Pexels-Fallback")
                video_clips = fetch_pexels_videos_multi(bildsuche, work_dir,
                                                        f"clip_{i:02d}", count=3,
                                                        exclude_ids=_used_vid_ids)
                img_paths_raw = [f"{work_dir}/slide_{i:02d}_img{j}.jpg"
                                 for j in range(num_imgs)]
                saved_imgs = fetch_pexels_images(bildsuche, img_paths_raw, orientation=orientation)
        else:
            # Klassischer Pfad: Pexels Videos + Bilder zuerst
            video_clips = fetch_pexels_videos_multi(bildsuche, work_dir,
                                                    f"clip_{i:02d}", count=3,
                                                    exclude_ids=_used_vid_ids)
            img_paths_raw = [f"{work_dir}/slide_{i:02d}_img{j}.jpg"
        ...
        # ── 2. Dynamic Segment (Bild-/Video-Mix mit Ken-Burns) ────────
    ...
    # Timeout grosszuegig: Video neu zu encodieren dauert auf der NAS oft >600s
    ...
    # Hintergrundmusik mischen
    final_out = f"{OUTPUT_DIR}/video_{datetime.date.today().isoformat()}_{fmt}.mp4"
...
# ─── THUMBNAIL ────────────────────────────────────────────────────
def build_thumbnail(script, work_dir):
    """Erzeugt ein kreatives 1280x720 YouTube-Thumbnail:
    KI-Hintergrund (Pollinations, Wochen-Kunststil) + grosser, plakativer
    Satire-Titel + Logo. Gibt den Pfad zur JPG-Datei zurueck (oder None)."""
    from PIL import Image, ImageDraw, ImageFont, ImageFilter
    W, H = 1280, 720
    art  = todays_art_style()

    thumb_text = (script.get("thumbnail_text")
                  or script.get("titel", "")).strip()[:60]
    bildsuche  = (script.get("thumbnail_bildsuche")
                  or script.get("titel", "health"))

    # 1) KI-Hintergrund generieren (im Wochen-Kunststil), sonst Verlauf
    bg_path = f"{work_dir}/thumb_bg.jpg"
    ai_prompt = (f"{art['prefix']}{bildsuche}, dramatic expressive close-up, "
                 f"bold lighting, eye-catching, high contrast, vibrant")
    bg = None
    if generate_ai_image(ai_prompt, bg_path, W, H):
        try:
            ...
            log.warning(f"Thumbnail-BG laden fehlgeschlagen: {e}")
            bg = None
    if bg is None:
        bg = Image.new("RGB", (W, H), (12, 35, 20))
        d  = ImageDraw.Draw(bg)
        for y in range(H):
            t = y / H
            d.line([(0, y), (W, y)], fill=(int(12+30*t), int(35+55*t), int(20+35*t)))

    # 2) Dunkles Gradient-Panel unten links fuer Lesbarkeit
    overlay = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    od = ImageDraw.Draw(overlay)
    od.rectangle([0, int(H*0.45), W, H], fill=(0, 0, 0, 150))
    bg = Image.alpha_composite(bg.convert("RGBA"), overlay).convert("RGB")

    # 3) Plakativer Text mit Kontur/Schatten
    ...
    # Textumbruch passend zur Breite
    words, lines, cur = thumb_text.upper().split(), [], []
    size = 120
    font = _font(size)
    max_w = int(W * 0.92)
    for w in words:
    ...
    if cur:
        lines.append(" ".join(cur))
    # Schriftgroesse reduzieren bis alles passt (max 4 Zeilen)
    while len(lines) > 4 and size > 60:
        size -= 10
        font = _font(size)
        words = thumb_text.upper().split(); lines, cur = [], []
        for w in words:
            test = " ".join(cur + [w])
            if draw.textlength(test, font=font) <= max_w:
                cur.append(w)
    ...
    accent = (210, 255, 120)  # callidus-gruen/limette
    for ln in lines:
    ...
    # Akzentbalken links
    draw.rectangle([0, H - total_h - int(H*0.09), int(W*0.025), H - int(H*0.04)], fill=accent)

    # 4) Logo oben rechts (falls vorhanden)
    try:
        if os.path.exists(LOGO_PATH):
            logo = Image.open(LOGO_PATH).convert("RGBA")
            lw   = int(W * 0.16)
            ratio = lw / logo.size[0]
            logo = logo.resize((lw, int(logo.size[1] * ratio)), Image.LANCZOS)
            bg.paste(logo, (W - lw - 30, 30), logo)
    except Exception as e:
        log.warning(f"Thumbnail-Logo fehlgeschlagen: {e}")

    # 5) Als JPG speichern (< 2 MB Pflicht bei YouTube)
    out = f"{work_dir}/thumbnail.jpg"
    q   = 92
    bg.save(out, "JPEG", quality=q)
    while os.path.getsize(out) > 1_900_000 and q > 50:
        q -= 8
        bg.save(out, "JPEG", quality=q)
    log.info(f"Thumbnail erstellt: {out} ({os.path.getsize(out)//1024} KB, Text: '{thumb_text}')")
    return out


def set_youtube_thumbnail(video_id, thumb_path):
    """Laedt ein Custom-Thumbnail fuer ein bereits hochgeladenes Video hoch.
    Benoetigt einen verifizierten YouTube-Kanal (Custom Thumbnails)."""
    try:
        ...
        log.info(f"Thumbnail hochgeladen fuer Video {video_id}")
        return True
    except Exception as e:
        log.error(f"Thumbnail-Upload fehlgeschlagen: {e}", exc_info=True)
        return False


# ─── FIRESTORE / CALLIDUS TV (Nexus App) ──────────────────────────
...
def save_to_callidus_tv(video_id, script):
    ...
    unter der Kategorie 'Callidus TV'. Loescht die aeltesten Eintraege,
    sobald mehr als CALLIDUS_TV_MAX Videos in dieser Kategorie vorhanden sind.
    """
    try:
        token   = _firestore_token()
        headers = {"Authorization": f"Bearer {token}", "Content-Type": "application/json"}
        order_val = int(time.time())
        ...
        r = requests.post(f"{FIRESTORE_BASE}/videos", json=doc, headers=headers, timeout=15)
        ...
        rq   = requests.post(f"{FIRESTORE_BASE}:runQuery", json=query, headers=headers, timeout=15)
...
def run_workflow(force=False):
    # Wochentag-Check (Sonntag = Langvideo) – mit force=True (manuelles /jetzt) umgehbar
    if not force and datetime.date.today().weekday() not in POSTING_TAGE:
        log.info(f"Kein Posting-Tag heute – beende.")
        tg_send(f"⏭️ Heute kein Langvideo-Tag. Naechster Run: Sonntag um {DAILY_HOUR:02d}:{DAILY_MINUTE:02d} Uhr.\n(Mit /jetzt trotzdem sofort starten.)")
        return
    log.info("=" * 50)
    log.info("Workflow gestartet")
    art = todays_art_style()
    tg_send("🚀 <b>Callidus YT Workflow gestartet</b>\nGeneriere Sonntags-Langvideo (Momus-Stil)...")

    day   = datetime.date.today().toordinal()
    thema = THEMEN[day % len(THEMEN)]
    log.info(f"Thema: {thema}")
    tg_send(f"📋 <b>Thema:</b> {thema}\n🎨 <b>Kunststil:</b> {art['label']}\n🎬 <b>Format:</b> YouTube Langvideo (4-5 Min, Satire + Aufklaerung)")

    work_dir = tempfile.mkdtemp(dir=BASE_DIR)
    try:
        tg_send("✍️ Generiere Script (Satire → Aufklaerung)...")
        script = generate_script(thema)
        tg_send(f"📝 <b>Titel:</b> {script['titel']}\n\n⏳ Erstelle Video + Thumbnail (ca. 5-10 Min)...")
        ...
        if approved:
            ...
            # ── Kreatives Thumbnail erstellen & hochladen ──────────
            try:
                tg_send("🖼️ Erstelle & lade kreatives Thumbnail hoch...")
                thumb_path = build_thumbnail(script, work_dir)
                if thumb_path and set_youtube_thumbnail(video_id, thumb_path):
                    tg_send("✅ <b>Thumbnail:</b> hochgeladen")
                else:
                    tg_send("⚠️ <b>Thumbnail:</b> Upload fehlgeschlagen (Video ist trotzdem live)")
            except Exception as e:
                log.error(f"Thumbnail-Schritt Fehler: {e}", exc_info=True)
                tg_send(f"⚠️ Thumbnail-Fehler: {str(e)[:200]}")

            # ── Callidus TV: In Nexus App speichern ────────────────
            tg_send("📱 Speichere in Callidus TV (Nexus App)...")
            if save_to_callidus_tv(video_id, script):
...
def bot_mode():
    ...
    _singleton_lock = acquire_singleton_lock("callidus_main")  # nur 1 Instanz
    tg_send(
        "🤖 <b>Callidus Langvideo-Bot gestartet!</b>\n\n"
        f"📅 Automatisch <b>Sonntags um {DAILY_HOUR:02d}:{DAILY_MINUTE:02d} Uhr</b> (Langvideo)\n\n"
        "Befehle:\n"
        ...
        try:
            ...
            if r.status_code == 409:
                log.error("Telegram 409 Conflict: Eine ZWEITE Bot-Instanz pollt denselben Token! "
                          "Bitte alle main.py-Prozesse beenden und nur einen starten.")
                time.sleep(15)
                continue
            r.raise_for_status()
            updates = r.json().get("result", [])
            ...
            for update in updates:
                ...
                if text in ("/jetzt", "jetzt"):
                    ...
                    # Auf naechsten Posting-Tag (Sonntag) vorruecken
                    for _ in range(8):
                    ...
                    tg_send(
                        ...
                        "/status – Bot-Status anzeigen\n"
                        "/hilfe – Diese Hilfe\n\n"
                        f"⏰ Auto-Run Sonntags um {DAILY_HOUR:02d}:{DAILY_MINUTE:02d} Uhr\n"
                        f"📺 Format: Langvideo (Satire+Aufklaerung) → YouTube + Thumbnail + Callidus TV"
                    )

            # Woechentliche automatische Ausfuehrung (Sonntag)
            now   = datetime.datetime.now()
            today = now.date()
            if (now.hour   == DAILY_HOUR
                ...
                workflow_running = True
                log.info("Woechentlicher automatischer Workflow (Sonntag)")
                tg_send(f"⏰ <b>Sonntags-Start</b> ({DAILY_HOUR:02d}:{DAILY_MINUTE:02d} Uhr)")
                try:
                    run_workflow()
                finally:
                    workflow_running = False
